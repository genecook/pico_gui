/**
  ******************************************************************************
  * @file    my_fatfs_storage.c - hacked for experimentation
...
extern uint8_t aBuffer[1440];/* 480 * 3 = 1440 */
...
*/	
	...
	aBuffer[index] = 0;
                ...
                int bmp_background_color = 65529;
		int xcolor = 0;
      
		for (i = 0; i < height; i ++) {
			  f_read(&file1, aBuffer, 92, (UINT *)&BytesRead);
			  //f_read(&file1, aBuffer + 45, 45, (UINT *)&BytesRead);
			  ...
			  for (j = 0; j < width; j++) {
				  ...
				  if (xcolor == 0) {
			          ...
			          if (pic[i*240+j] == bmp_background_color) {
                                    pic[i*240+j] = YELLOW;
				    //printf("background color: %u, desired color: %u\n",background_color,YELLOW);
			          }
			  }
		}
...
* @retval err: Error status (0=> success, 1=> fail)
*/
uint32_t my_Storage_CopyFile(const char* BmpName1, const char* BmpName2)
{
  uint32_t index = 0;
  FIL file1, file2;
...
* @retval err: Error status (0=> success, 1=> fail)
*/
uint32_t my_Storage_CheckBitmapFile(const char* BmpName, uint32_t *FileLen)
{
    uint32_t err = 0;
...
*/
uint32_t my_Storage_GetDirectoryBitmapFiles(const char* DirName, char* Files[])
{
	uint32_t i = 0, j = 0;
	FRESULT res;
...
/**
  ...
  */
uint8_t my_Buffercmp(uint8_t* pBuffer1, uint8_t* pBuffer2, uint16_t BufferLength)
{
  uint8_t ret = 1;
  while (BufferLength--)
  {
